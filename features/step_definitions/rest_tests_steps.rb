...
Then(/^проверяю (наличие|отсутствие) логина (\w+\.\w+) в списке пользователей$/) do |presence, login|
  if presence == 'наличие'
...
And(/^(позитивно|негативно) удаляю пользователя с логином (\w+\.\w+)$/) do |type, login|
  begin
...
Then(/^проверяю соответствие данных пользователя эталону:$/) do |data_table|
  user_data = data_table.raw
